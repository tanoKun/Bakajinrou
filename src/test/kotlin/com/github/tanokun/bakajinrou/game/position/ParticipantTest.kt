...
class ParticipantTest {
    object TempPosition: Position {
        ...
        override fun doAtVictoryFinishing() = Unit

        override fun doAtLoseFinishing() = Unit
    }

    @Test
    @DisplayName("生存状態から死亡状態へ遷移できる")
    fun canChangeSurvivedToDead() {
        val player = Participant(UUID.randomUUID(), TempPosition)
    ...
    fun changeSuspendedToSurvived() {
        val player = Participant(UUID.randomUUID(), TempPosition).apply { suspended() }
    ...
    fun cannotChangeDeadToAnotherState() {
        val player = Participant(UUID.randomUUID(), TempPosition).apply { dead() }
    ...
    fun cannotChangeSameState() {
        val survived = Participant(UUID.randomUUID(), TempPosition)
        assertFalse(survived.survived())

        val dead = Participant(UUID.randomUUID(), TempPosition).apply { dead() }
        assertFalse(dead.dead())

        val suspended = Participant(UUID.randomUUID(), TempPosition).apply { suspended() }
