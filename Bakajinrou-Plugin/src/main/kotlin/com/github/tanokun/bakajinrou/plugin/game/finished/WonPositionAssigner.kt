...
import com.github.tanokun.bakajinrou.plugin.game.finished.finisher.SystemFinishNotifier
import com.github.tanokun.bakajinrou.plugin.game.finished.finisher.each.CitizenSideFinishNotifier
import com.github.tanokun.bakajinrou.plugin.game.finished.finisher.each.FoxSideFinishNotifier
import com.github.tanokun.bakajinrou.plugin.game.finished.finisher.each.WolfSideFinishNotifier
...
class WonPositionAssigner(
    ...
    private fun observeWin(wonInfo: WonInfo) {
        val finisher = when (wonInfo) {
            is WonInfo.Won -> when (wonInfo.side) {
                Side.WEREWOLF -> WolfSideFinishNotifier(playerProvider, translator, audience)
                Side.VILLAGE -> CitizenSideFinishNotifier(playerProvider, translator, audience)
                Side.FOX -> FoxSideFinishNotifier(playerProvider, translator, audience)
            }
