...
class TabListModifier(
    ...
    fun modifyByUpdateGameMode(viewerUniqueId: ParticipantId, contents: List<PlayerInfoData>): List<PlayerInfoData> {
        ...
        return contents.map { entry ->
            val gameType =
                if (isVisibleSpectator(viewer, entry.profileId.asParticipantId())) entry.gameMode else EnumWrappers.NativeGameMode.SURVIVAL
    ...
    fun modifyByUpdateDisplayName(viewerPlayer: Player, contents: List<PlayerInfoData>): List<PlayerInfoData>  {
        ...
        return contents.map { entry ->
            val gameType = if (isVisibleSpectator(viewer, entry.profileId.asParticipantId())) entry.gameMode else EnumWrappers.NativeGameMode.SURVIVAL
