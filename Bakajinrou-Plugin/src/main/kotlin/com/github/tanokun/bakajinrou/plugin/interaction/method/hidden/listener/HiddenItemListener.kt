package com.github.tanokun.bakajinrou.plugin.interaction.method.hidden.listener
...
import com.github.tanokun.bakajinrou.plugin.interaction.method.adapter.ItemViewer.isVisible
import com.github.tanokun.bakajinrou.plugin.interaction.method.hidden.HideItem
...
class HiddenItemListener(
    ...
    register(packet = PacketType.Play.Server.ENTITY_EQUIPMENT, listenerPriority = ListenerPriority.LOW) { event, packet, receiver ->
        val receiver = jinrouGame.getParticipant(receiver.uniqueId.asParticipantId()) ?: return@register
    ...
    register(packet = PacketType.Play.Client.USE_ENTITY, listenerPriority = ListenerPriority.LOW) { event, packet, sender ->
        if (!jinrouGame.existParticipant(sender.uniqueId.asParticipantId())) return@register
