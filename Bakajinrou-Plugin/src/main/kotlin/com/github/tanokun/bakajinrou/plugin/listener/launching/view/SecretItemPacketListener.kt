...
class SecretItemPacketListener(
    ...
    register(packet = PacketType.Play.Server.ENTITY_EQUIPMENT, listenerPriority = ListenerPriority.LOW) { event, packet, receiver ->
        ...
        val modifiedItems = items.mapNotNull { pair ->
            ...
            if (receiver.isDead()) return@mapNotNull pair
    ...
    register(packet = PacketType.Play.Client.USE_ENTITY, listenerPriority = ListenerPriority.LOW) { event, packet, sender ->
        if (jinrouGame.getParticipant(sender.uniqueId) == null) return@register

        val item = sender.inventory.itemInMainHand
        if (item.persistentDataContainer.getOrDefault(isVisibleKey, PersistentDataType.BOOLEAN, true)) return@register

        val action = packet.enumEntityUseActions.read(0)
        if (action == WrappedEnumEntityUseAction.attack()) event.isCancelled = true
    }
})*/
